use gtk::prelude::*;
use gtk::{TreeViewColumn, CellRenderer, TreeModel, TreeIter};
use ffi;
use glib_ffi;
use glib;
...
pub(crate) trait TreeViewColumnSetCellDataFuncExt {
...
impl<O: IsA<TreeViewColumn> + IsA<glib::object::Object>> TreeViewColumnSetCellDataFuncExt for O {
    fn set_cell_data_func<'a, P, F>(&self, cell_renderer: &P, closure: F)
        where P: IsA<CellRenderer>,
        F: Fn(&Self, &CellRenderer, &TreeModel, &TreeIter) + 'static
    {
...
unsafe extern fn destroy_notify(_p: glib_ffi::gpointer) {}
